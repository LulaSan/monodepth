...
class MonodepthModel(object):
    ...
    def __init__(self, params, mode, left, right, M, reuse_variables=None, model_index=0):
        self.params = params
        self.mode = mode
        self.left = left
        self.right = right
        self.M = M 
        #M_array=self.build_M()
        print("######################################################## M SHAPE AFTER PROCESSING SHOULD BE 256,512)")
        print(M.shape)
...
#        a_img_l=self.M
#        a_img_int=a_img_l[:,:,0]
#        a_img_fl=tf.divide(a_img_l[:,:,2],100)
#        a_img_revenge=tf.add(a_img_int,a_img_fl)
#        return a_img_revenge
    ...
    def SSIM(self, x, y):
        ...
        mu_x = slim.avg_pool2d(x, 3, 1, 'SAME') #€ra ovunque "VALID" ma cambiava la shape
    ...
    def WSSIM(self,x,y):
        ...
        SSIM= tf.image.ssim(x, y, max_val=255, filter_size=11, filter_sigma=1.5, k1=C1, k2=C2)
        return tf.clip_by_value((1 - SSIM) / 2, 0, 1)
    ...
    def build_outputs(self):
        self.M_4D = self.M[..., tf.newaxis]
        self.M4= tf.image.resize_images(self.M_4D,[32,64], tf.image.ResizeMethod.AREA)
        self.M3= tf.image.resize_images(self.M_4D, [64,128], tf.image.ResizeMethod.AREA)
        self.M2= tf.image.resize_images(self.M_4D, [128,256], tf.image.ResizeMethod.AREA)
        self.M_scale = [self.M_4D, self.M2, self.M3, self.M4]
        # STORE DISPARITIES
    ...
    def build_losses(self):
        with tf.variable_scope('losses', reuse=self.reuse_variables):
            ...
            # L1 M
            self.l1_left = [ self.M_scale[i]* tf.abs(self.left_est[i] - self.left_pyramid[i]) for i in range(4) ]
            ...
            # SSIM M
            self.ssim_left = [self.M_scale[i]* self.SSIM( self.left_est[i],  self.left_pyramid[i]) for i in range(4)]
            ...
            #WSSIM
            #self.ssim_left = [self.WSSIM( self.left_est[i],  self.left_pyramid[i]) for i in range(4)]
            #self.ssim_loss_left  = [tf.reduce_mean(s) for s in self.ssim_left]
            #self.ssim_right = [self.WSSIM(self.right_est[i], self.right_pyramid[i]) for i in range(4)]
            #self.ssim_loss_right = [tf.reduce_mean(s) for s in self.ssim_right]
            ...
            # DISPARITY SMOOTHNESS M
            self.disp_left_loss  = [tf.reduce_mean(self.M_scale[i][:,:,:-1,:]* tf.abs(self.disp_left_smoothness[i]))  / 2 ** i for i in range(4)]
            self.disp_right_loss = [tf.reduce_mean(tf.abs(self.disp_right_smoothness[i])) / 2 ** i for i in range(4)]
            self.disp_gradient_loss = tf.add_n(self.disp_left_loss + self.disp_right_loss)

            # LR CONSISTENCY M
            self.lr_left_loss  = [tf.reduce_mean(self.M_scale[i] * tf.abs(self.right_to_left_disp[i] - self.disp_left_est[i]))  for i in range(4)]
